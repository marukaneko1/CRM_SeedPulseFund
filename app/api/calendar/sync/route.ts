...
export async function POST(request: NextRequest) {
  try {
    ...
    if (syncGoogle) {
      ...
      // const google = await fetchGoogleCalendarEvents(session.user.googleToken)
      
      // Demo: Add sample Google Calendar events
      const googleEvents = [
        {
          id: `google-${Date.now()}-1`,
          title: 'Team Standup (Google)',
          description: 'Daily team sync - synced from Google Calendar',
        ...
        },
        {
          ...
          description: 'Quarterly product review - synced from Google Calendar',
